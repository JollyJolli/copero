...
export function showAchievementCelebration(entries = []) {
  ...
  host.innerHTML = `<style>@keyframes cee-achievement-in{0%{opacity:0;transform:translateY(24px) scale(.88)}55%{opacity:1;transform:translateY(-3px) scale(1.02)}100%{opacity:1;transform:translateY(0) scale(1)}}@keyframes cee-achievement-out{to{opacity:0;transform:translateY(-12px) scale(.96)}}#${HOST_ID}{position:fixed;right:24px;bottom:24px;z-index:2147483647;pointer-events:none;font-family:ui-sans-serif,system-ui,sans-serif}#${HOST_ID} article{display:flex;align-items:center;gap:12px;min-width:270px;max-width:340px;padding:12px 16px;border:1px solid rgba(0,224,224,.4);border-radius:16px;color:#efffff;background:linear-gradient(135deg,rgba(0,20,20,.98),rgba(0,70,70,.98));box-shadow:0 18px 45px rgba(0,0,0,.52),0 0 28px rgba(0,151,151,.14),inset 0 1px rgba(0,224,224,.12);animation:cee-achievement-in .48s cubic-bezier(.22,.9,.3,1) both}#${HOST_ID} i{display:grid;place-items:center;width:46px;height:46px;flex:none;border-radius:13px;color:#dfffff;background:linear-gradient(145deg,#00baba,#006b6b);box-shadow:0 0 20px rgba(0,151,151,.22),inset 0 1px rgba(255,255,255,.2);font-size:22px;font-style:normal;letter-spacing:-5px;padding-right:5px}#${HOST_ID} small,#${HOST_ID} strong{display:block}#${HOST_ID} small{margin-bottom:3px;color:#68eeee;font-size:9px;font-weight:900;letter-spacing:.12em}#${HOST_ID} strong{font-size:14px;line-height:1.15}</style><article><i>${preview}</i><div><small>${subtitle}</small><strong>${title}</strong></div></article>`;
